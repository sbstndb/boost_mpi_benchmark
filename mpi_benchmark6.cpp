...
void benchmark_raw_mpi_vector(int rank, int size, int num_iterations) {
    ...
    if (rank == 0) {
	    ...
	    for( int dest = 1 ; dest < size ; dest++){
	            ...
	            // Envoi des données
	            for (int j = 0; j < outer_size; j++) {
	                MPI_Send(vec.data[j].data(), inner_sizes[j], MPI_INT, dest, 2 + j, MPI_COMM_WORLD);
	            }
	    }
        }
        int ack;
...
void benchmark_pack_mpi_vector(int rank, int size, int num_iterations) {
    ...
    if (rank == 0) {
	    ...
	    for (int dest = 1 ; dest < size ; dest++){
	            MPI_Send(&position, 1, MPI_INT, dest, 0, MPI_COMM_WORLD);
	            MPI_Send(buffer.data(), position, MPI_PACKED, dest, 1, MPI_COMM_WORLD);
	    }
        }
        int ack;
	for (int dest = 1 ; dest < size ; dest++){
...
void benchmark_datatype_mpi_vector(int rank, int size, int num_iterations) {
    ...
    if (rank == 0) {
		...
		for (int dest = 1 ; dest < size ; dest++){
	                MPI_Send(vec.data[j].data(), 1, inner_type, dest, 2 + j, MPI_COMM_WORLD);
		}
